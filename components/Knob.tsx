...
const Knob: React.FunctionComponent<Props & WithJUCESliderProps> = ({label, parameterID, 
    color, display, roundFunction, displayFunction, style, value, properties, onChange, 
    ...
    if (!displayFunction) {
        displayFunction = (value: number) => `${value.toFixed(0)} Hz`
    }

    if (display === "q") {
        displayFunction = (value: number) => `${value.toFixed(2)} Q`
    }
